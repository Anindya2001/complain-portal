...
const { Users, Complains, Replies, ForbiddenWords } = require("../db");
...
router.post("/updatefilterwords", authMiddleware, async (req, res) => {
  try {
    const inputWords = req.body.filterwords;
    for (let i = 0; i < inputWords.length; i++) {
      const word = inputWords[i];
      await ForbiddenWords.create({ words: word });
    }
