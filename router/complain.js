...
const { Complains, ForbiddenWords } = require("../db");
...
router.post("/createcomplain", authMiddleware, async (req, res) => {
  ...
  const forbiddenWords = await ForbiddenWords.find({});
  const filterWords = [];
  forbiddenWords.forEach((word) => {
    filterWords.push(word.words);
    console.log(word.words);
  });
  console.log(filterWords);
  const complainWords = req.body.title
    .split(" ")
    .concat(req.body.description.split(" "));
  const forbiddenWordFound = isFound(complainWords, filterWords);
  if (forbiddenWordFound)
    return res
      .status(403)
      .json({ msg: "Your complain contains some forbidden words" });
